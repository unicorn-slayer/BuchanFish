...
{  
    ...
    magicBitboard->init_sliders_attacks(BISHOP);
    //magicBitboard->init_sliders_attacks(ROOK); NEEDS FIXED
}

void Engine::initBoard(Bitboard& board)
{
...
void Engine::setPiece(Bitboard& board, int square, int pieceType, int pieceColor)
{
    board.pieces[pieceColor][pieceType] |= (1ULL << square);
}

void Engine::unsetPiece(Bitboard& board, int square, int pieceType, int pieceColor)
{
    board.pieces[pieceColor][pieceType] &= ~(1ULL << square);
}

void Engine::setupStartingPosition(Bitboard& board)
{
    initBoard(board);
...
void Engine::printBoard(const Bitboard& board)
{
...
bool Engine::isMoveLegal(const Bitboard& board, int sourceSquare, int destinationSquare)
{
...
int Engine::convert(std::string number)
{
...
int Engine::makeMove(Bitboard& board, int sourceSquare, int destinationSquare)
{
...
int Engine::evaluatePosition(Bitboard& board)
{
...
std::vector<Move> Engine::generateMoves(const Bitboard& board, int player)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        for (int j = 1; j < 7; j++)
        {
            occupancy |= board.pieces[i][j];

            if (i == 0)
            {
                whiteOccupancy |= board.pieces[i][j];
            }
            else
            {
                blackOccupancy |= board.pieces[i][j];
            }
        }
    } //occupancy is a bitboard containing all occupied squares
    ...
    for (int sourceSquare = 0; sourceSquare < 64; sourceSquare++) {
        ...
        if (board.pieces[player][PAWN] & (1ULL << sourceSquare)) {
            int moveOffsets[4] = { 8, 16, 7, 9 };

            for (int offset : moveOffsets)
            {
                if (Engine::isMoveLegal(board, sourceSquare, (player == WHITE) ? (sourceSquare + offset) : (sourceSquare - offset)))
                {
                    moves.push_back({ sourceSquare, (player == WHITE) ? (sourceSquare + offset) : (sourceSquare - offset) });
                }
            }
            continue;
        }
        if (board.pieces[player][KNIGHT] & (1ULL << sourceSquare))
        {
            ...
            for (int offset : moveOffsets)
            {
                if (Engine::isMoveLegal(board, sourceSquare, (sourceSquare + offset)))
                {
        ...
        if (board.pieces[player][BISHOP] & (1ULL << sourceSquare))
        {
            ...
            //U64 convertedOccupancy = 0;

            //for (int i = 0; i < 64; i++)
            //{
            //    U64 number = 0;

            //    if (get_bit(occupancy, i))
            //    {
            //        number = 1ULL << (i ^ 56);
            //        convertedOccupancy |= number;
            //    }
            //}

            /////

            //int convertedSource = sourceSquare ^ 56;

            //bishopAttacks = magicBitboard->get_bishop_attacks(convertedSource, convertedOccupancy);
            bishopAttacks = magicBitboard->get_bishop_attacks(sourceSquare, occupancy);

            if (player == 0)
            {
                bishopAttacks = bishopAttacks & (~whiteOccupancy);
            }
            else
            {
                bishopAttacks = bishopAttacks & (~blackOccupancy);
            }

            while (MagicBitboards::get_ls1b_index(bishopAttacks) != -1)
            {
                int destSquare = MagicBitboards::get_ls1b_index(bishopAttacks); //INFINITE LOOP :  get index of least significant bit, then get the next

                pop_bit(bishopAttacks, destSquare);

                moves.push_back({ sourceSquare, destSquare });
            } //TEST CODE TEST CODE (need to make sure that the last square is in fact takeable)
            ...
            //    while ((Engine::isMoveLegal(board, sourceSquare, targetSquare)) == true)
            //    {
            //        moves.push_back({ sourceSquare, targetSquare }); //need to do bitboard operation here, i.e. get bishop attacks bitboard
            //        // and convert into a move, with source and target squares, and add them. Basically skip all the isMoveLegal stuff.
            //        //Do this by finding the index of each bit that is turned on, and set this equal to dest square, then add to movelist.
        ...
        {
            // Define the possible queen move directions
            int moveDirections[8] = { -1, 1, 8, -8, -9, -7, 7, 9 };

            // Generate moves for each possible move direction
            for (int direction : moveDirections) {
        ...
        if (board.pieces[player][KING] & (1ULL << sourceSquare))
        {
            ...
            for (int direction : moveDirections) {
                ...
                if ((Engine::isMoveLegal(board, sourceSquare, targetSquare)) == true)
                {
                    moves.push_back({ sourceSquare, targetSquare });
                }
            }
...
int Engine::minimax(Bitboard& board, int depth, int alpha, int beta, bool maximizingPlayer)
{
    ...
    if (maximizingPlayer) {
        ...
        std::vector<Move> moves = Engine::generateMoves(board, WHITE);
        ...
        std::vector<Move> moves = Engine::generateMoves(board, BLACK);
...
void Engine::computerMakeMove(Bitboard& board, int currentPlayer)
{
    // Generate all possible moves for the current player
    std::vector<Move> moves = Engine::generateMoves(board, currentPlayer);
